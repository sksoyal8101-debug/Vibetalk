...
export function Admin() {
  ...
  return (
    <div className="space-y-5">
            ...
            <div className="max-h-[46vh] space-y-2 overflow-y-auto">
              {db.giftLog.slice(0, 16).map((g) => (
                <div key={g.id} className="flex items-center gap-2.5 rounded-2xl border border-white/8 bg-white/[0.03] p-2.5">
                  ...
                  <span className="shrink-0 text-[10px] text-white/30">{timeAgo(g.at)} ago</span>
                </div>
              ))}
            </div>
          </Card>
      ...
      {panel === "events" && (
        <Card className="!rounded-[28px] p-4">
          <SectionHeader title="Events" subtitle={`${db.events.length} scheduled`} />
          {db.events.length === 0 ? (
            <EmptyState icon={<Bell className="size-6" />} title="No events" body="Hosts can publish events from the Events screen." action={<Link to="/events"><Button>Open events</Button></Link>} />
          ) : (
            <div className="grid gap-2.5 sm:grid-cols-2">
              {db.events.map((e) => (
                <div key={e.id} className="rounded-2xl border border-white/8 bg-white/[0.03] p-3">
                  <p className="truncate text-[13px] font-bold">{e.name}</p>
                  <p className="mt-1 text-[11px] text-white/45">{e.date} {e.time} · {e.rsvps} RSVPs · @{userById(e.hostId)?.username ?? "you"}</p>
                </div>
              ))}
            </div>
          )}
      ...
      {panel === "notifications" && (
        <Card className="!rounded-[28px] p-4">
          <SectionHeader title="Alert log" subtitle="What the demo pushed to the member" icon={<Bell className="size-4.5 text-vibe-200" />} />
          <div className="space-y-2">
            {db.notifications.slice(0, 12).map((n) => (
              <div key={n.id} className="flex items-start gap-3 rounded-2xl border border-white/8 bg-white/[0.03] p-3">
                <span className={cn("mt-1 size-2 shrink-0 rounded-full", n.read ? "bg-white/25" : "bg-blush-400")} />
                <div className="min-w-0 flex-1">
                  <p className="truncate text-[13px] font-bold">{n.title}</p>
                  <p className="mt-0.5 text-[11px] text-white/45">{n.body}</p>
                </div>
                <span className="shrink-0 text-[10px] uppercase tracking-widest text-white/25">{n.type}</span>
              </div>
            ))}
            {social.admin.announcements.map((a) => (
              <div key={a.id} className="rounded-2xl border border-coin-400/25 bg-coin-500/[0.06] p-3">
                <p className="text-[11px] font-black uppercase tracking-widest text-coin-400">Admin announcement</p>
                <p className="mt-1 text-[12.5px] text-white/75">{a.text}</p>
              </div>
            ))}
          </div>
        </Card>
      ...
      {panel === "analytics" && (
        <div className="grid gap-4 lg:grid-cols-2">
          <Card className="!rounded-[28px] p-5">
            <SectionHeader title="Per-user activity" subtitle="Demo dataset spread" />
            <div className="h-[240px]">
              <Bar
                data={{
                  labels: db.users.slice(0, 8).map((u) => u.username.slice(0, 8)),
                  datasets: [{ label: "Messages", data: db.users.slice(0, 8).map((u) => db.chats.filter((c) => c.userId === u.id).length + db.messages.filter((m) => m.from === u.id).length + 4), backgroundColor: "rgba(124,58,237,.6)", borderRadius: 8, borderSkipped: false as const }],
                }}
                options={{ responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { grid: { color: "rgba(255,255,255,.06)" }, ticks: { color: "rgba(255,255,255,.45)", font: { size: 9 } } }, y: { grid: { color: "rgba(255,255,255,.06)" }, ticks: { color: "rgba(255,255,255,.45)", font: { size: 10 }, precision: 0 } } } }}
              />
            </div>
          </Card>
          <Card className="!rounded-[28px] p-5">
            ...
            {!social.posts.length ? (
              ...
              <div className="grid grid-cols-2 gap-2.5">
                {[
                  { l: "Posts", v: stats.posts }, { l: "Clips", v: stats.videos }, { l: "Stories", v: social.stories.length },
                  { l: "Groups", v: social.groups.length }, { l: "Group msgs", v: social.groupMessages.length }, { l: "Follows", v: db.follows.length },
                ].map((s) => (
                  <div key={s.l} className="rounded-2xl border border-white/8 bg-white/[0.03] p-3">
                    <p className="font-display text-xl font-extrabold">{s.v}</p>
                    <p className="text-[10px] font-bold uppercase tracking-widest text-white/40">{s.l}</p>
                  </div>
                ))}
              </div>
            )}
